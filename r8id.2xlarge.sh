...
# --- Swap setup: 32GB swapfile on local NVMe, swappiness=8 ---
if [ -n "$EPHEMERAL_DISK" ] && [ -d "/mnt/build" ]; then
    SWAP_FILE="/mnt/build/swapfile"
else
    SWAP_FILE="/swapfile"
fi
...
if ! grep -qs "vm.swappiness" /etc/sysctl.conf; then
    echo "vm.swappiness=8" | sudo tee -a /etc/sysctl.conf
else
    sudo sed -i 's/^vm.swappiness=.*/vm.swappiness=8/' /etc/sysctl.conf
fi
